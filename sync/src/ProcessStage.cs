...
public class ProcessStage(FetchResult FetchedData) : Stage<ProcessResult?>("process")
{
	...
	{
		var streamTasks = FetchedData.StreamsResponses.Select(gameStreams => computeStreams(gameStreams.Streams));
		var userTasks = FetchedData.UsersResponse.Select(gameUsers => computeUsers(gameUsers.Users));
		var userResults = new List<UserProcessResult>();
		var streamResults = new List<StreamProcessResult>();

		foreach (var streamTask in streamTasks)
		{
			streamResults.Add(await streamTask);
		}

		foreach (var userTask in userTasks)
		{
			userResults.Add(await userTask);
		}
